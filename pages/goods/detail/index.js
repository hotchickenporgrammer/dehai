...
userinfo: function (options) {
    ...
    if (options.detail.userInfo) t.getUserInfo(this.onShow(), null, {
        iv: a,
        encryptedData: e
    });
},
  ...
  getshoptips: function(event){
    var that = this;
    var id = event.currentTarget.dataset.id;
    e.get("goods/getshoptips", {'id':id}, function (a) {
      that.setData({
        shiptipsid: a.shoptips,
        shoptipsshow: 1
      });
    });
  },
  shoptipshide:function(){
    this.setData({
      shiptipsid: '',
      shoptipsshow: 0
    });
  },
  selecttips: function (event){
    var that = this;
    var id = event.currentTarget.dataset.id;
    console.log(event.currentTarget.dataset.id)
    e.get("goods/goodsremind", {'id':id}, function (a) {
      if(a.error==0){
        that.setData({
          'goods.goodsremind': 1
        });
