...
Page({
  ...
  submit: function () {
    ...
    if (0 == this.data.goods.giftid && 1 == this.data.goods.gifts.length && (d = this.data.goods.gifts[0].id),
      ...
      if (t.shoptips && t.selecttips == '0') return void a.alert("请勾选" + t.shoptips.title+"!");
      ...
      a.post("order/create/submit", o, function (t) {
        ...
        if(t.isiden=='1'){
          0 == t.error ? wx.redirectTo({
            url: "/pages/order/pay/contract?id=" + t.orderid
          }) : a.alert(t.message);
        } else {
          0 == t.error ? wx.redirectTo({
